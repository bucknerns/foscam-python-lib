...
class FoscamCamera(requests.Session):
    def __init__(self, login_url, usr, pwd, daemon=False, verbose=True):
        super(FoscamCamera, self).__init__()
        # requests variables cert verify false and default url params
        self.verify = False
        self.params = {"usr": usr, "pwd": pwd}

        self.login_url = login_url
        self.daemon = daemon
        self._log = logging.getLogger("FoscamCamera")
        if verbose:
            self._log.addHandler(logging.StreamHandler())
            self._log.setLevel(0)
            self._log.parent.setLevel(0)

    def send_command(self, cmd, params=None, raw=False):
        url = "{0}/cgi-bin/CGIProxy.fcgi".format(self.login_url)
        params = params or {}
        params["cmd"] = cmd
        r = self.get(url, params=params)
        self._log.debug("Send Foscam command: {0}".format(r.url))
        if raw:
            self._log.debug("Returning raw Foscam response: len={0}".format(
                len(r.content)))
        try:
            root = ET.fromstring(r.content)
            r.fos_result = int(root.find("result").text)
            r.entity = {e.tag: e.text for e in root if e.text != "result"}
        except:
            r.entity = None
            r.fos_result = None
        self._log.debug("Received Foscam entity: {0}".format(r.entity))
        return r
    ...
    def execute_command(self, cmd, params=None, callback=None, raw=False):
        ...
        def execute_with_callbacks(cmd, params=None, callback=None, raw=False):
            r = self.send_command(cmd, params, raw)
            if callback:
                callback(r.entity.get("result"), r.entity)
            return r

        if self.daemon:
            t = Thread(
                target=execute_with_callbacks, args=(cmd, ),
    ...
    def set_ip_info(
        self, is_dhcp, ip="", gate="", mask="", dns1="", dns2="",
            callback=None):
        """
        isDHCP: 0(False), 1(True)
        System will reboot automatically to take effect after
        call this CGI command.
        """
        params = {
            "isDHCP": is_dhcp,
            "ip": ip,
    ...
    def set_port_info(
            self, webport, mediaport, httpsport, onvifport, callback=None):
        """
    ...
    def set_wifi_setting(
        self, ssid, psk, isenable, isusewifi, nettype, encryptype, authmode,
        keyformat, defaultkey, key1="", key2="", key3="", key4="", key1len=64,
            key2len=64, key3len=64, key4len=64, callback=None):
        """
        Set wifi config.
        ...
        return self.execute_command(
            "setWifiSetting", params, callback=callback)
    ...
    def set_ddns_config(
        self, isenable, hostname, ddnsserver, user, password,
            callback=None):
        """
        Set DDNS config.
        """
    ...
    def set_sub_video_stream_type(self, format, callback=None):
        ...
        return self.execute_command(
            "setSubVideoStreamType", params, callback=callback)
    ...
    def set_sub_stream_format(self, format, callback=None):
        ...
        params = {"format": format}
        return self.execute_command(
            "setSubStreamFormat", params, callback=callback)
    ...
    def get_main_video_stream_type(self, callback=None):
        ...
        """
        return self.execute_command(
            "getMainVideoStreamType", callback=callback)
    ...
    def set_main_video_stream_type(self, streamtype, callback=None):
        ...
        return self.execute_command(
            "setMainVideoStreamType", params, callback=callback)
    ...
    def change_user_name(self, usrname, newusrname, callback=None):
        ...
        Change user name.
        """
        params = {"usrName": usrname, "newUsrName": newusrname}
        r = self.execute_command("changeUserName", params, callback=callback)
        if usrname == self.params.get("usr") and r.entity["result"] == 0:
            self.params["usr"] = newusrname
        return r
    ...
    def change_password(self, usrname, oldpwd, newpwd, callback=None):
        ...
        Change password.
        """
        params = {"usrName": usrname, "oldPwd": oldpwd, "newPwd": newpwd}
        r = self.execute_command("changePassword", params, callback=callback)
        if usrname == self.params.get("usr") and r.entity["result"] == 0:
            if oldpwd == self.params.get("pwd"):
                self.params["pwd"] = newpwd
        return r

    # *************** Device manage *******************

    def set_system_time(
        self, time_source, ntp_server, date_format, time_format, time_zone,
            is_dst, dst, year, mon, day, hour, minute, sec, callback=None):
        """
        Set systeim time
        """
    ...
    def set_ptz_speed(self, speed, callback=None):
        ...
        """
        return self.execute_command(
            "setPTZSpeed", {"speed": speed}, callback=callback)
    ...
    def get_ptz_preset_point_list(self, callback=None):
        ...
        """
        return self.execute_command(
            "getPTZPresetPointList", callback=callback)
    ...
    def set_alarm_record_config(
        ...
        return self.execute_command(
            "setAlarmRecordConfig", params, callback=callback)
    ...
    def get_local_alarm_record_config(self, callback=None):
        ...
        """
        return self.execute_command(
            "getLocalAlarmRecordConfig", callback=callback)

    def set_local_alarm_record_config(
        self, is_enable_local_alarm_record=1, local_alarm_record_secs=30,
            callback=None):
        """
    ...
    def get_schedule_record_config(self, callback=None):
        ...
        """
        return self.execute_command(
            "getScheduleRecordConfig", callback=callback)
    ...
    def snap_picture_2(self, callback=None):
        ...
        """
        return self.execute_command(
            "snapPicture2", callback=callback, raw=True)
